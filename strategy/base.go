...
func NewBaseStrategy(ID string, input string, output string, prefix string, retrySeconds int64, maxRetryTimes int) *BaseStrategy {
...
func (b *BaseStrategy) Execute() error {
	...
	log.Infoln(fmt.Sprintf("服务(%v)启动: %s -> %s", b.ID, b.Input, b.OutputDir()))
	...
	for progress := range output {
		if progress.FramesProcessed != "" {
			...
			log.Infof("服务(%s)-保存流成功, Process = %+v", b.ID, progress)
		} else {
			log.Warningf("服务(%s)-保存流失败, Process = %+v", b.ID, progress)
		}
	}
